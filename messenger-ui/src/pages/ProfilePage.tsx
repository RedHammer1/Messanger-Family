...
const ProfilePage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: user?.phone || '',
    bio: user?.bio || '',
    is_phone_visible: user?.is_phone_visible || false,
    is_email_visible: user?.is_email_visible || false
  });
  ...
  const handleChangeTag = async () => {
    ...
    try {
      ...
      setEditingTag(false);
      updateUser({ ...user, tag: tagData.tag });
    } catch (err) {
      setError('Ошибка соединения с сервером');
    } finally {
      setLoading(false);
  ...
  return (
    <div className="page-container">
      <PageHeader title="Профиль" subtitle="Управление личной информацией" />
