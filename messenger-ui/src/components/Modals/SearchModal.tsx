...
const SearchModal = ({ currentUserId, onClose, onOpenChat }: SearchModalProps) => {
    ...
    const searchMessages = async () => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:3306/api/chats/${currentUserId}/search/messages?q=${encodeURIComponent(query)}`);
    ...
    const searchChats = async () => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:3306/api/chats/${currentUserId}/search/chats?q=${encodeURIComponent(query)}`);
            const data = await response.json();
            setChatResults(data);
        } catch (err) {
